...
from prettyconf.sources import RecursiveFileSearchSource


def test_config_file_search(create_file, files_path):
    create_file(files_path + '/../.env')
    create_file(files_path + '/../setup.txt')  # invalid settings
    create_file(files_path + '/../settings.ini')
    file_search = RecursiveFileSearchSource(
        patterns=['.env', '*.ini'],
        start_path=os.path.dirname(files_path))

    assert repr(file_search).startswith('RecursiveFileSearchSource(starting_path=')
    assert len(file_search.filenames) == 2  # 2 *valid* files created
